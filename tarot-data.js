...
function cardBackSVG(){
  ...
  return `<svg class="card-back-svg" viewBox="0 0 100 150" xmlns="http://www.w3.org/2000/svg" preserveAspectRatio="xMidYMid slice">
    ...
    <g stroke-linejoin="round" stroke-linecap="round">
      ...
      <!-- 整只猫居中于月盘：等比例放大 1.28 倍（高度充满月亮），以猫中心(50,59.7)为基准 -->
      <g transform="translate(50,66) scale(1.28) translate(-50,-59.7)">
      <!-- 优雅卷尾：贴合身形、尾尖内收上扬（先画，置于身后；随加宽的腿部外移） -->
      <path d="M60.5 82 C 69 82, 72 73, 68 67 C 66 64, 62 65, 61.5 68"
            fill="none" stroke="${G}" stroke-width="2.2" stroke-linecap="round"/>
      <!-- 整猫闭合剪影：高尖耳 → 窄脸（头部宽度不变）→ 肩部加宽 → 腿部(下半身)再加宽到 1.3 倍 → 端坐 -->
      <path d="M50 51
               C 47.4 51, 45.7 49.6, 45 47
               L 43.9 39.4
               C 43.7 38, 44.4 37.6, 45.3 38.6
               C 46.8 40.3, 48.1 42.6, 48.9 45.3
               C 49.4 45.1, 50.6 45.1, 51.1 45.3
               C 51.9 42.6, 53.2 40.3, 54.7 38.6
               C 55.6 37.6, 56.3 38, 56.1 39.4
               L 55 47
               C 54.3 49.6, 52.6 51, 50 51
               C 52.9 51.5, 55.2 53.4, 56.4 56
               C 59.5 60, 59.8 64.5, 59.1 69
               C 58.7 72.5, 59 76.5, 60.1 80
               C 60.7 81.6, 59.4 82.4, 57.2 82.4
               L 51.7 82.4
               C 51 81.8, 50.5 81.4, 50 81.4
               C 49.5 81.4, 49 81.8, 48.3 82.4
               L 42.8 82.4
               C 40.6 82.4, 39.3 81.6, 39.9 80
               C 41 76.5, 41.3 72.5, 40.9 69
               C 40.2 64.5, 40.5 60, 43.6 56
               C 44.8 53.4, 47.1 51.5, 50 51 Z"
            fill="${G}" stroke="${Gdk}" stroke-width="0.8" stroke-linejoin="round"/>
      <!-- 前腿分隔暗线，强化端坐的两条前腿（随腿部加宽外移） -->
      <path d="M46.4 60 C 45.9 68, 45.9 76, 46.4 81.6" fill="none" stroke="${Gdk}" stroke-width="0.6" opacity="0.5"/>
      <path d="M53.6 60 C 54.1 68, 54.1 76, 53.6 81.6" fill="none" stroke="${Gdk}" stroke-width="0.6" opacity="0.5"/>
      </g>
      <!-- 月顶星点 -->
