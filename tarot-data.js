...
function cardBackSVG(){
  ...
  return `<svg class="card-back-svg" viewBox="0 0 100 150" xmlns="http://www.w3.org/2000/svg" preserveAspectRatio="xMidYMid slice">
    ...
    <g stroke-linejoin="round" stroke-linecap="round">
      ...
      <!-- ===== 月中央：一只「朝左的优雅坐姿猫」金色单线条剪影（参考线稿） =====
           连笔走线：尖耳 → 闭眼侧脸 → 鼻尖 → 下巴 →（背脊一气呵成）→ 臀 → 后腿落地 → 前腿 → 胸 → 颈，
           外加身前优雅卷起的尾巴。线条悬于明亮月面之上。 -->
      <!-- 身后淡淡暗晕，强化「月中剪影」的层次 -->
      <ellipse cx="52" cy="70" rx="20" ry="22" fill="${NAVY2}" opacity="0.14"/>
      <g fill="none" stroke="${G}" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round">
        <!-- 主体轮廓：头顶→双耳→侧脸→下巴→喉→胸→前腿→地面→后腿→臀→背脊→回到颈 -->
        <path d="M47 47
                 C 46 43, 45.5 41, 44.5 41
                 C 43.8 42.4, 43.4 44.6, 43 46.4
                 C 42 47, 41 47.8, 40.2 48.8
                 C 38 51, 35.6 53, 34 55.6
                 C 33 57.2, 33.4 58.6, 35 58.8
                 C 36.4 59, 37.6 59.8, 38.4 61.2
                 C 39.2 62.6, 39.4 64.6, 39.4 67
                 C 39.4 74, 39.4 82, 39.6 88.4
                 C 39.7 90.2, 41 90.6, 42 89.4
                 C 43 88.2, 43 86.4, 43 84
                 C 47 86, 53 86.4, 58 84.4
                 C 60.5 91, 62.6 91, 63.4 89
                 C 64 87.4, 63.2 84, 62.4 80
                 C 65.5 72, 65.5 62, 60 54
                 C 56 48, 51.6 46.6, 47 47 Z"/>
        <!-- 左耳内折一笔 -->
        <path d="M44 42.4 C 44.6 44, 45.4 45.4, 46.4 46.4"/>
        <!-- 右耳（背脊侧那只尖耳） -->
        <path d="M48 47 C 49.5 43.5, 51 41.5, 52.4 41.2
                 C 52 43.4, 51.4 45.6, 50.4 47.4"/>
      </g>
      <!-- 尾巴：身前优雅卷成一圈（最具辨识度的一笔） -->
      <path d="M40 86
               C 33 88, 27 90, 25 84
               C 23.6 79.6, 27 75.6, 31.4 76.4
               C 34.6 77, 35.4 80.6, 33 82
               C 31.4 82.9, 29.6 81.8, 30 80"
            fill="none" stroke="${G}" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/>
      <!-- 闭眼（上扬一弧）与鼻尖小点 -->
      <path d="M37.6 53.4 q2.4 -1.4 4.4 0.2" fill="none" stroke="${Gdk}" stroke-width="1" stroke-linecap="round"/>
      <circle cx="34.2" cy="56" r="0.9" fill="${RED}" opacity="0.8"/>
      <!-- 胡须（自鼻吻向左伸出） -->
      <g stroke="${Gdk}" stroke-width="0.6" opacity="0.55" stroke-linecap="round">
        <path d="M34.6 57.4 q-6 0.4 -10 -1.4"/>
        <path d="M35 58.6 q-6 1.4 -10 2"/>
      </g>
      <!-- 月顶星点 -->
      <circle cx="55" cy="40" r="1.4" fill="${G}" stroke="${Gdk}" stroke-width="0.5"/>
      ${star}
